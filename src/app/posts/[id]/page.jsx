...
`;
      ...
      try {
        const { data } = await client.query({ query, variables: { slug: params?.id } });
        // const { data } = await client.query({ query })
        // console.log(data.categories.edges[0].node.posts)
        // setPosts(data.categories)
        setPosts(data.categories.edges[0].node.posts.nodes);
    ...
    <>
      ...
      <nav className='bg-blue-900 text-white border-gray-200 px-2 sm:px-4 py-2.5 dark:bg-blue-900'>
        <span className='self-center text-lg font-semibold whitespace-nowrap dark:text-white'>
          ระบบประชาสัมพันธ์ มทร.ศรีวิชัย ข่าวของหน่วยงาน{' '}
          <span className='text-yellow-400'> [{categoriesName}] </span>{' '}
          ระบบทดสอบ
        </span>
      </nav>
      {/* <div className='main'>
